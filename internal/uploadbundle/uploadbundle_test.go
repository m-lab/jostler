...
func TestNew(t *testing.T) { //nolint:paralleltest
	...
	tests := []struct {
		name          string
		wdClient      watchdir.WatchDirClient
		gcsBucket     string
		gcsDataDir    string
		gcsBaseID     string
...
func TestBundleAndUploadWatchChan(t *testing.T) { //nolint:paralleltest
	saveGCSClient := GCSClient
	GCSClient = testhelper.DiskNewClient
	defer func() {
		GCSClient = saveGCSClient
	...
	// BundleAndUpload() returns when its watch channel is closed.
	setupDataDir(t)
	sizeMax := uint(100)
	ageMax := 2 * time.Second
	wdClient, ubClient := setupClients(t, sizeMax, ageMax)
	go func() {
		<-time.After(ageMax + 2*time.Second)
		t.Logf(">>> closing watch channel")
		close(wdClient.WatchChan())
	}()
	if err := ubClient.BundleAndUpload(context.Background()); err != nil {
		t.Fatalf("New() = %v, want nil", err)
	}
}

func TestBundleAndUploadAgeChan(t *testing.T) { //nolint:paralleltest
	saveGCSClient := GCSClient
	GCSClient = testhelper.DiskNewClient
	defer func() {
		GCSClient = saveGCSClient
	...
	// BundleAndUpload() returns when its age channel is closed.
	setupDataDir(t)
	sizeMax := uint(100)
	ageMax := 2 * time.Second
	_, ubClient := setupClients(t, sizeMax, ageMax)
	go func() {
		<-time.After(ageMax + 2*time.Second)
		t.Logf(">>> closing age channel")
		close(ubClient.ageChan)
	}()
	if err := ubClient.BundleAndUpload(context.Background()); err != nil {
		t.Fatalf("New() = %v, want nil", err)
	}
...
func setupClients(t *testing.T, sizeMax uint, ageMax time.Duration) (watchdir.WatchDirClient, *UploadBundle) { //nolint:ireturn
	t.Helper()
