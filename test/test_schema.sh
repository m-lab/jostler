...
readonly MLAB_SANDBOX="mlab-sandbox"

readonly BQ_TABLES=(
	#"${MLAB_SANDBOX}:${USER}.hello1"
	#"${MLAB_SANDBOX}:${USER}.foo1"
	#"${MLAB_SANDBOX}:ndt_raw.scamper1" # has standard columns
...
main() {
	...
	for table in "${BQ_TABLES[@]}"; do
		...
		create_datatype_schema "${project_id}" "${dataset}" "${datatype}"
...
create_datatype_schema() {
	...
	# Does the datatype schem file already exist?
...
# create a table with it.  If the table alreay exists, it must be
# first deleted.
#
...
verify_table_schema() {
	...
	if bq show --project_id "${MLAB_SANDBOX}" "${dataset}.${datatype}" >/dev/null 2>/dev/null; then
		echo Deleting the existing "${MLAB_SANDBOX}:${dataset}.${datatype}" table
		# Delete the table (bq rm prompts the user to verify).
		execute bq rm --project_id "${MLAB_SANDBOX}" --table "${dataset}.${datatype}"
	fi

	# Create a table with the schema that jostler saved.
	execute bq mk --project_id "${MLAB_SANDBOX}" --table "${dataset}.${datatype}" "${datatype}.json"

	# Show the newly created table's full schema.
	execute bq show --project_id "${MLAB_SANDBOX}" --format prettyjson "${dataset}.${datatype}"
